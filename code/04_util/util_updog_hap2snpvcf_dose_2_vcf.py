...
def convert_dosage2vcf(updog_dose, new_vcf_header, vcf_readCount, hh_dict, bias_dict, od_dict, pmc_dict, updog_gt):
    ...
    while line:
        if line.startswith("#CHROM"):
            ...
            line_array = line.strip().split()
            if line_array[2] in hh_dict:
                outp.write('\t'.join(line_array[:8]) + ';HH=' + hh_dict[line_array[2]] + ';BIAS=' + bias_dict[line_array[2]] + ';OD=' + od_dict[line_array[2]] + ';PMC=' + pmc_dict[line_array[2]] + '\tGT:DP:RA:AD')
                vcf_index = 9
