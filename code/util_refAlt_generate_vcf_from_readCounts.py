    ...
    while line:
        ...
        if line_array[0] != '' and line_array[0] != '*':
            if line_array[0] == 'AlleleID':
                ...
                if len(line_array[1:]) != len(set(line_array[1:])):
                    samples = []
                    for i in line_array[1:]:
                        if i in samples:
                            ...
                            print('# Rename sample: ', i, 'to', i_updated)
                        else:
                            outp.write('\t' + i)
                        samples.append(i)
                    outp.write('\n')
                else:
                    samples = line_array[1:]
                    outp.write('\t'.join(line_array[1:]) + '\n')
            else:
                ...
                if ref_line_array[0].split('|')[0] == alt_line_array[0].split('|')[0]:
                    ref_dp = sum(list(map(int, ref_line_array[1:])))
                    alt_dp = sum(list(map(int, alt_line_array[1:])))
                    dp = ref_dp + alt_dp
...
if __name__=='__main__':
    ...
    parser.add_argument('input',
                        help='DArTag read count CSV input file - Processed report with the first column markerIDs and first row sample IDs')
