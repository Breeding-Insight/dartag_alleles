...
PROCESS_README='/Users/dz359/PycharmProjects/BI/blueberry_dartag_00_microhaplotype_db/data/blueberry_allele_db_v019_process.readme'
NO_NEW_ALLELE_README='/Users/dz359/PycharmProjects/BI/blueberry_dartag_00_microhaplotype_db/data/blueberry_allele_db_v018_DBlue24-9195_noNewAllele.readme'
exec &> $PROCESS_README

# Note: change here ######
SCRIPTS_DIR='/Users/dz359/PycharmProjects/BI/01_dartag_alleles/code'
ALLELE_DB_DIR='/Users/dz359/PycharmProjects/BI/blueberry_dartag_00_microhaplotype_db/data'
ALLELE_DB='blueberry_allele_db_v018.fa'
MATCHCNT_LUT='blueberry_allele_db_v018_matchCnt_lut.txt'
REPORT='/Users/dz359/PycharmProjects/BI/blueberry_dartag_blueCran2_5plates/data/Report-DBlue24-9195/DBlue24-9195_MADC.csv'


# For panels designed for 54 bp amplicons, but sequenced to 81-bp from DArTag MADC reports
printf "\n# Update snpIDs in DArTag reports\n"
python $SCRIPTS_DIR/util_madc_update_snpID.py $ALLELE_DB_DIR/20200819-BI-Blueberry_10K_SNPs_forDArT_3K_snpID_lut.csv $REPORT
...
# Note that some microhaplotypes may be shorter than 54 bp after removing adaptor
# These are dropped from the output file
...
#### ! Don't need BLAST now since we are using SeqIO and Bio for checking sequence identity #############
...
if test -f "$CUTADAPT_UNI"; then
    python $SCRIPTS_DIR/step05_parse_madc_allele81bp_seqIO.py $ALLELE_DB_DIR/$MATCHCNT_LUT $ALLELE_DB_DIR/$ALLELE_DB $TMP_RENAME_UPDATED $CUTADAPT_UNI $ALLELE_DB_DIR/$README
else
    python $SCRIPTS_DIR/step05_parse_madc_allele81bp_seqIO.py $ALLELE_DB_DIR/$MATCHCNT_LUT $ALLELE_DB_DIR/$ALLELE_DB $TMP_RENAME $CUTADAPT $ALLELE_DB_DIR/$README
fi
MADC_CLEANED=${REPORT_SNPID%????}'_rename_updatedSeq.csv'


printf "\n#  4). Check if there is a new version DB\n"
...
if test -f "$ALLELE_DB_DIR/$ALLELE_DB_NEW"; then
    ...
    printf "\n#  5). Check if there are duplicates."
    ...
    printf "\n#  6). If there are duplicates in the new version DB, update MADC after removing duplicated alleles in db\n"
